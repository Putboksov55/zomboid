...
def print_percentage_results(results: Dict[str, float], filter_name: str = None):
    if filter_name:
        print(f"Відсотки за умовами для предметів з ім'ям '{filter_name}':")
    else:
        print("Відсотки за умовами для всіх предметів:")
    
    for condition, percentage in results.items():
        print(f"{condition}: {percentage:.2f}%")

def main():
    parser = argparse.ArgumentParser(description="Аналіз даних із файлу.")
    parser.add_argument("file_path", type=Path, help="Шлях до файлу (JSON, CSV, XML).")
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--percentage-all", action="store_true", help="Показати відсотки за умовами для всіх.")
    group.add_argument("--percentage-name", type=str, help="Показати проценти за умовами для предметів із зазначеним ім'ям.")
